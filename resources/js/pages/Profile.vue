    ...
    <AccountShell title="Профайл">
        <div class="card mt-6 p-6">
            <div class="flex items-center gap-4">
                ...
                <div><p class="font-mono font-semibold tracking-wider">+976 {{ auth.user.phone }}</p><div class="mt-1 flex flex-wrap gap-1.5"><span class="badge" :class="ROLE[auth.user.role].cls">{{ ROLE[auth.user.role].label }}</span><span v-if="auth.user.is_verified" class="badge bg-emerald-100 text-emerald-800">✓ verify.mn баталгаажсан</span><router-link v-else-if="auth.isCustomer" :to="{ name: 'verify' }" class="badge bg-amber-100 text-amber-800">Баталгаажаагүй →</router-link></div></div>
            </div>
            <dl v-if="auth.user.is_verified" class="mt-5 grid gap-3 rounded-xl bg-cream-100 p-4 text-sm sm:grid-cols-2"><div><dt class="text-stone-500">Баталгаажсан дугаар</dt><dd class="font-mono font-medium">{{ auth.user.verified_phone }}</dd></div><div><dt class="text-stone-500">Баталгаажуулагч</dt><dd class="font-medium">verify.mn (SMS 144773)</dd></div></dl>

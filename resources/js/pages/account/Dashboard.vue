    ...
    <AccountShell>
        ...
        <div v-else class="space-y-6">
            ...
            <div v-if="!auth.isVerified" class="flex flex-col items-start gap-4 rounded-2xl bg-gradient-to-r from-amber-500 to-orange-500 p-5 text-white sm:flex-row sm:items-center">
                ...
                <div class="flex-1"><p class="font-bold">Бүртгэлээ баталгаажуулна уу</p><p class="text-sm text-amber-50">Захиалга өгөхийн өмнө verify.mn-ээр нэг удаа баталгаажуулах шаардлагатай.</p></div>
                <router-link :to="{ name: 'verify' }" class="btn bg-white text-amber-700 hover:bg-amber-50">Баталгаажуулах</router-link>
            </div>
            <div v-else class="flex items-center gap-3 rounded-2xl bg-emerald-50 px-5 py-3 text-sm text-emerald-800 ring-1 ring-emerald-200"><ShieldCheckIcon class="h-5 w-5" /> Бүртгэл verify.mn-ээр баталгаажсан · {{ auth.user.last_name }} {{ auth.user.first_name }}</div>
            ...
            <div class="grid gap-6 xl:grid-cols-2">
                ...
                <div class="card p-5">
                    ...
                    <ul v-else class="mt-3 space-y-4">
                        <li v-for="o in d.active_deliveries" :key="o.id" class="rounded-xl bg-stone-50 p-4">
                ...
                <div class="card p-5">
                    ...
                    <ul v-else class="mt-3 divide-y divide-stone-100 text-sm">
                        <li v-for="i in d.backorder_items" :key="i.id" class="flex items-center gap-3 py-2.5"><img :src="i.image" class="h-10 w-10 rounded-lg bg-stone-100 object-cover" alt="" /><div class="flex-1"><p class="font-medium">{{ i.product_name }}</p><p class="text-xs text-stone-500">{{ i.variant_label }} × {{ i.quantity }} · {{ i.order?.order_number }}</p></div><span class="badge bg-amber-100 text-amber-800">Хүлээгдэж буй</span></li>
                    </ul>
                </div>
            </div>
            ...
            <div class="card">
                ...
                <ul v-else class="divide-y divide-stone-100">
                    <li v-for="o in d.recent_orders" :key="o.id"><router-link :to="{ name: 'order', params: { number: o.order_number } }" class="flex flex-wrap items-center gap-4 px-5 py-3 hover:bg-stone-50">
                        <div class="flex -space-x-2"><img v-for="i in o.items.slice(0, 3)" :key="i.id" :src="i.image" class="h-9 w-9 rounded-lg bg-stone-100 object-cover ring-2 ring-white" alt="" /></div>

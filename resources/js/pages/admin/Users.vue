...
const form = reactive({ name: '', phone: '', email: '', role: 'customer', password: '', is_active: true });

async function load() { result.value = (await api.get('/admin/users', { params: f })).data; }
function create(role = 'customer') { editing.value = null; Object.assign(form, { name: '', phone: '', email: '', role, password: '', is_active: true }); open.value = true; }
function edit(u) { editing.value = u; Object.assign(form, { name: u.name, phone: u.phone || '', email: u.email || '', role: u.role, password: '', is_active: u.is_active }); open.value = true; }
async function save() {
    saving.value = true;
...
<template>
    <div>
        ...
        <div class="card mt-5 flex flex-wrap items-center gap-3 p-4">
            <input v-model="f.q" placeholder="Нэр, утас, имэйл..." class="input max-w-xs py-2" />
            <div class="flex gap-1 rounded-xl bg-cream-200/60 p-1 text-sm">
                <button v-for="(r, k) in { '': 'Бүгд', ...Object.fromEntries(Object.entries(ROLE).map(([k, v]) => [k, v.label])) }" :key="k" @click="f.role = k" class="rounded-lg px-3 py-1.5 font-medium" :class="f.role === k ? 'bg-paper shadow-sm' : 'text-stone-500'">{{ r }}</button>
            </div>
        ...
        <div v-else class="card mt-4 overflow-x-auto">
            <table class="min-w-full text-sm">
                <thead class="bg-cream-100 text-left text-xs uppercase text-stone-500"><tr><th class="px-4 py-3">Хэрэглэгч</th><th class="px-4 py-3">Эрх</th><th class="px-4 py-3">Баталгаажуулалт</th><th class="px-4 py-3">И-мэйл</th><th class="px-4 py-3">Захиалга / Хүргэлт</th><th class="px-4 py-3">Төлөв</th><th class="px-4 py-3">Бүртгүүлсэн</th><th></th></tr></thead>
                <tbody class="divide-y divide-stone-100">
                    <tr v-for="u in result.data" :key="u.id" class="hover:bg-cream-100">
                        <td class="px-4 py-3"><div class="flex items-center gap-3"><span class="flex h-9 w-9 items-center justify-center rounded-full bg-brand-100 font-bold text-brand-700">{{ u.name.slice(0, 1) }}</span><div><p class="font-medium">{{ u.name }}</p><p class="font-mono text-xs text-stone-400">{{ u.phone }}<span v-if="u.email"> · {{ u.email }}</span></p></div></div></td>
                        <td class="px-4 py-3"><StatusBadge :value="u.role" type="role" /></td>
                        <td class="px-4 py-3"><template v-if="u.is_verified"><span class="badge bg-emerald-100 text-emerald-800">✓ Баталгаажсан</span><p class="mt-0.5 font-mono text-xs text-stone-400">{{ u.verified_phone }}</p></template><span v-else-if="u.role === 'customer'" class="badge bg-amber-100 text-amber-800">Баталгаажаагүй</span><span v-else class="text-xs text-stone-400">—</span></td>
                        <td class="px-4 py-3 text-stone-600">{{ u.email || '—' }}</td>
        ...
        <Modal :open="open" :title="editing ? 'Хэрэглэгч засах' : 'Шинэ хэрэглэгч'" @close="open = false">
            <form @submit.prevent="save" class="space-y-4">
                ...
                <div><label class="label">Утасны дугаар *</label><input v-model="form.phone" class="input font-mono" required inputmode="tel" placeholder="99001122" /></div>
                <div class="grid grid-cols-2 gap-3"><div><label class="label">И-мэйл</label><input v-model="form.email" type="email" class="input" /></div><div><label class="label">Эрх *</label><select v-model="form.role" class="input"><option v-for="(r, k) in ROLE" :key="k" :value="k">{{ r.label }}</option></select></div></div>

            ...
            <div class="mt-6 grid gap-6 xl:grid-cols-[1fr_380px]">
                <div class="space-y-6">
                    ...
                    <div class="grid gap-6 md:grid-cols-2">
                        ...
                        <div class="card p-5 text-sm"><h3 class="mb-3 font-semibold">Хүргэлтийн хаяг</h3><p class="font-medium">{{ order.shipping_name }} · {{ order.shipping_phone }}</p><p class="text-stone-600">{{ order.shipping_city }}<span v-if="order.shipping_district">, {{ order.shipping_district }}</span><span v-if="order.shipping_khoroo">, {{ order.shipping_city === 'Улаанбаатар' ? order.shipping_khoroo + '-р хороо' : order.shipping_khoroo + ' баг' }}</span></p><p class="text-stone-600">{{ order.shipping_address }}</p><p v-if="order.note" class="mt-2 rounded-lg bg-amber-50 p-2 text-xs text-amber-800">📝 {{ order.note }}</p><p v-if="order.courier_note" class="mt-2 rounded-lg bg-sky-50 p-2 text-xs text-sky-800">🛵 {{ order.courier_note }}</p></div>
                    </div>

...
| Хэсэг | Боломж |
| --- | --- |
| **Дэлгүүр (хэрэглэгч)** | Нүүр, ангилал, хайлт, шүүлт (ангилал / хямдрал / төлөв / өнгө / хэмжээ / багц / үнэ), эрэмбэ, хуудаслалт |
...
| **SMS баталгаажуулалт** | Бүртгэлтэй дугаараас нь 144773 руу нэг удаагийн 6 оронтой код SMS-ээр илгээж дугаараа баталгаажуулна (Mobile-Originated SMS). Баталгаажаагүй бол захиалга өгөх боломжгүй. API түлхүүргүй үед `VERIFY_MOCK=true` локал симуляц |
...
| **Хаягийн дэвтэр** | Хүргэлтийн хаягууд хэрэглэгч дээр хадгалагдана (үндсэн хаяг, олон хаяг); checkout дээр сонгох эсвэл шинээр нэмнэ. Улаанбаатарын 9 дүүрэг + хороо, 21 аймаг + 330 сумын бүрэн жагсаалт (`resources/data/mn-locations.json`), сервер талд шалгагдана |
| **Захиалга** | Хадгалсан эсвэл шинэ хаяг, тэмдэглэл, зөвхөн QPay төлбөр, цуцлах, түүх/timeline |
...
| **Хүргэлтийн ажилтан** | Өөрт томилогдсон хүргэлтүүд, статистик, статус шинэчлэх (хүлээн авсан → замд → хүргэсэн / амжилтгүй) |
...
## SMS баталгаажуулалтын тохиргоо

Mobile-Originated SMS баталгаажуулалт. Хэрэглэгч утаснаасаа **144773** дугаарт нэг удаагийн кодыг илгээж, бид `GET /sessions/{id}` дээр `sessionStatus === "VERIFIED"` болсныг шалгаж баталгаажуулна.

```env
VERIFY_MN_API_KEY=...                     # SMS үйлчилгээ үзүүлэгчийн dashboard-аас (заавал, хэзээ ч commit хийхгүй)
...
Урсгал: `POST /api/verify/start {phone}` → `POST /sessions` (шинэ санамсаргүй 6 оронтой код) → хэрэглэгчид `displayInstruction`-ийг үгчлэн харуулж, `smsUri` (sms:144773?body=код) товч санал болгоно → SPA 3 секунд тутам `GET /api/verify/sessions/{id}/check` дуудна (`GET /sessions/{id}`) → `VERIFIED` болмогц polling зогсоно; хугацаа (`expiresAt`, 300с) дууссан бол шинэ код үүсгэнэ. Callback тохируулсан бол үйлчилгээ үзүүлэгч `GET /api/verify/callback/{token}` руу дуудна — бид шууд 200 буцааж, хариу илгээсний дараа `GET /sessions/{id}`-ээр дахин шалгана (callback-д өөрт нь итгэхгүй).
...
app/Services/VerifyService.php SMS баталгаажуулалт (verifyPhone)
